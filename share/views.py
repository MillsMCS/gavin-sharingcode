...
def show_script(request, script_id):
    if request.method == "GET":
        ...
        if not user.is_authenticated:
            ...
            user_review = Review.objects.filter(coder=user.id).filter(script=script.id)  # script, coder, stars, feedback fields
            print(user_review)
...
def create_script(request,problem_id):
    if request.method == "POST":
        ...
        if not request.POST["title"]:  # this does not work with textarea  form elements
            return render(request, "share/build_script_form.html",
            {"user": user, "problem":problem, "error":"One of the required fields was empty"})
        else:
            coder = user.coder
            title = request.POST["title"]
            description = request.POST["description"]
